        ...
        try:
            ...
            if clean_text in ["آمار گپ", "آمار گروه"]:
                ...
                try:
                    ...
                    async for m in self.client.iter_participants(entity):
                        members.append(m)
                    ...
                    try:
                        ...
                        print("ENTITY ERROR:", repr(e))
                    print("CLIENT METHODS:")
                    print([x for x in dir(self.client) if "chat" in x.lower() or "group" in x.lower() or "member" in x.lower() or "participant" in x.lower()])
                    try:
                        full = await self.client.get_full_chat(chat_id)
                        print("FULL CHAT:", full)
                        print("FULL ATTRS:", dir(full))
                    except Exception as e:
                        print("FULL CHAT ERROR:", repr(e))
                    try:
