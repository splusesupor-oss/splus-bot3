...
# سکوت کاربر با ریپلای
            ...
            try:
                ...
                # فلود فقط برای یک کاربر بررسی شود، نه کل گروه
                if user_id not in self.flood_messages:
                    self.flood_messages[user_id] = []

                self.flood_messages[user_id].append(
                ...
                self.flood_messages[user_id] = [
                    x for x in self.flood_messages[user_id]
                    if now - x[0] <= 10
                ]

                # فقط اگر یک کاربر 5 پیام سریع بفرستد
                if len(self.flood_messages[user_id]) >= 20:
                    ids = [
                        x[1]
                ...
                try:
                    removed = await self.admin_actions.send_warning(
                        chat_id,
                        user_id,
                        username
                    )
            ...
            try:
                if getattr(event.message, "fwd_from", None) or getattr(event.message, "forward", None):
                    await self.client.delete_messages(
                        chat_id,
                        [event.message.id]
                    )
            ...
            # بررسی اسپم تکراری چندخطی داخل یک پیام
            try:
                import re
                ...
                if len(lines) >= 10:
                    ...
                    # اگر تعداد زیادی خط تکرار شده باشد
                    for line in unique_lines:
                        if lines.count(line) >= 3:
                            repeat_score += lines.count(line)
                ...
                if repeat_score >= 10:
                    ...
                    print(
                        f"🚫 MULTI LINE SPAM REMOVE {username} {user_id}"
                    )
            ...
            # ─── مسیر ۱: فیلتر کلمات ممنوعه گروه (کاملاً مستقل از اسپم) ───
            if await self._check_group_words(chat_id, message_text, username, event):
                return

            # ─── مسیر ۲: تشخیص اسپم (فقط اگر فیلتر گروه فعال نشد) ───
    ...
    async def _check_group_words(self, chat_id, message_text, username, event) -> bool:
        """
        فقط کلمات ممنوعه گروه را بررسی می‌کند.
        هیچ ارتباطی به سیستم اسپم، tracker یا punish ندارد.
        Returns: True اگر پیام فیلتر شد (caller باید return کند)
        """
        try:
            ...
            for word in group_words:
                if word and word in message_text:
                    print(f"🟡 GROUP WORD FILTER: '{word}' در پیام {username}")
                    ...
                    await self.admin_actions.send_warning(
                        chat_id=chat_id,
                        username=username,
                        reason=f"کلمه ممنوعه گروه: {word}",
                        count=0,
                        threshold=0,
                        reply_to=None
                    )
            ...
            self.logger.log_error(f"خطای _check_group_words: {e}")
