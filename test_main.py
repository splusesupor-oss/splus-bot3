            ...
            if clean_text in ["آمار گپ", "آمار گروه"]:
                ...
                try:
                    try:
                        ...
                        print("MEMBER COUNT API ERROR:", repr(e))
                        ...
                        try:
                            ...
                            print("MEMBER FALLBACK ERROR:", repr(e2))
                            member_count = 0
                ...
                await event.reply(
                    make_report(chat_id, member_count)
                )
...
# سکوت کاربر با ریپلای
            ...
            try:
                if is_admin(chat_id, username):
                    print(f'FINAL ADMIN BYPASS: {username}')
                    is_spam = False
                    reason = ''
            except Exception as e:
                print('FINAL ADMIN CHECK ERROR:', e)
            ...
            if is_spam:
                ...
                if self.config_manager.get("delete_spam", True):
                    ...
                    await self.smart_delete_spam(chat_id, user_id)
    ...
    async def smart_delete_spam(self, chat_id, spammer_id):
        import asyncio
        ...
        while empty_rounds < 10:
            try:
                ...
                print("SMART DELETE:", e)
                break

        print(f"SMART DELETE FINISHED ({deleted} deleted)")
        return deleted


    async def is_cached_admin(self, chat_id, user_id):
        import time

        try:
            now = time.time()

            # اگر کش کمتر از ۵ دقیقه است
            if chat_id in self.admin_cache:
                if now - self.admin_cache_time.get(chat_id, 0) < 300:
            ...
            for a in admins:
                if (
                    getattr(a, "creator", False)
                    or getattr(a, "admin_rights", None)
                ):
                    admin_ids.add(a.id)


            self.admin_cache[chat_id] = admin_ids
            self.admin_cache_time[chat_id] = now

            return user_id in admin_ids

        except Exception as e:
            print("ADMIN CACHE ERROR:", e)
            return False
