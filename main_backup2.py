    ...
    async def handle_new_message(self, event):
        ...
        try:
            ...
            if clean_text == "راهنما":
                ...
                # دستورها را از آخر متن پیدا می‌کنیم (جلوگیری از اشتباه فارسی)
                for word in ["پاک", "سکوت", "اخراج", "آزاد"]:
                    positions = []
                    start = 0

                    while True:
                        pos = help_text.find(word, start)
                        if pos == -1:
                            break
                    ...
                    if positions:
                        ...
                        # محاسبه offset برای پیام‌رسان‌ها (UTF-16)
                        offset = len(help_text[:pos].encode("utf-16-le")) // 2
            ...
            if event.is_private:
                ...
                if "صفر" in text:
                    try:
                        ...
                        nums = re.findall(r"(?<!\w)\d{5,}(?!\w)", text)
                        group_id = int(nums[-1]) if nums else None

                        print("ZERO DEBUG:", username, group_id)

                        if username and group_id:
                            user_id = None

                            counts = self.tracker.get_all_counts(group_id)

                            for uid in counts.keys():
                                try:
                                    user_obj = await self.client.get_entity(int(uid))
                                    if getattr(user_obj, "username", None) == username:
                                        user_id = int(uid)
                                        break
                                except Exception:
                                    continue
                            ...
                            if not user_id:
                                await event.reply("❌ این کاربر سابقه تخلف در این گروه ندارد")
                                return
