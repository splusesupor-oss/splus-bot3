...
class SoroushAntiSpamBot:
    ...
    async def handle_new_message(self, event):
        ...
        try:
            ...
            # اجازه بررسی گیف، استیکر و مدیا بدون متن
            if not message_text:
                if not getattr(event.message, "media", None):
                    return
                message_text = "[MEDIA]"

            # ضد گیف و استیکر پشت سرهم
            try:
                import time

                if not hasattr(self, "media_spam"):
                    self.media_spam = {}
                ...
                if is_media:
                    ...
                    if key not in self.media_spam:
                        self.media_spam[key] = []

                    self.media_spam[key].append((now, event.message.id))

                    self.media_spam[key] = [
                        x for x in self.media_spam[key]
                        if now - x[0] <= 15
                    ]

                    if len(self.media_spam[key]) >= 5:
                        ids = [x[1] for x in self.media_spam[key]]
                        ...
                        await event.reply(
                            f"⚠️ ارسال گیف/استیکر زیاد حذف شد ({count})"
                        )
                ...
                self.logger.log_error(f"خطای ضد گیف: {e}")
            ...
            if clean_text in ["فعال سازی", "غیر فعال"]:
                try:
                    ...
                    print(
    "OWNER DEBUG:", getattr(
        sender, "username", None), getattr(
            sender, "id", None), getattr(
...
# اخراج کاربر با ریپلای
    ...
    target, "username", None) or str(
        getattr(
            ...
            if clean_text in ["رفع سکوت", "آزاد"]:
                try:
    ...
    getattr(
        ...
        "username",
         None)):
                        await event.reply("❌ فقط ادمین‌ها اجازه دارند")
                        return
    ...
    'username',
     'کاربر')} برداشته شد"
                    ...
                    await event.reply(f"❌ خطا در رفع سکوت کاربر: {e}")
    ...
    getattr(
        ...
        'username',
         'کاربر')} سکوت شد"
            ...
            try:
                if getattr(event.message, "fwd_from", None):
                    forward_sender = getattr(
    event.message.fwd_from, "from_id", None)
    ...
    getattr(
        ...
        "channel_id",
         None)
            ...
            # بررسی اسپم
            # بررسی کلمات ممنوعه اختصاصی همین گروه
            group_words = get_words(chat_id)
            for gw in group_words:
                if gw and gw in message_text:
                    is_spam = True
                    reason = f"کلمه ممنوعه گروهی ({gw})"
                    break
            else:
    ...
    admin_id: int,
     chat_id: int):
        ...
        try:
            if cmd in ["فیلتر", "filterword"]:
                if len(parts) < 2:
                    await event.respond("❌ استفاده: فیلتر کلمه")
                    return

                word = " ".join(parts[1:])
                if add_word(chat_id, word):
                    await event.respond(f"✅ کلمه «{word}» در این گروه ممنوع شد")
                else:
                    await event.respond("⚠️ این کلمه قبلا برای این گروه ثبت شده")
                return

            elif cmd in ["رفع", "رفع_فیلتر", "unfilter"]:
                if len(parts) < 2:
                    await event.respond("❌ استفاده: رفع فیلتر کلمه")
                    return

                word = " ".join(parts[1:])
                if remove_word(chat_id, word):
                    await event.respond(f"✅ کلمه «{word}» از فیلتر گروه حذف شد")
                else:
                    await event.respond("⚠️ این کلمه وجود ندارد")
                return
    ...
    async def run(self):
        ...
        async def new_message_handler(event):
            ...
            if not event.is_private:
                ...
                if text not in ["فعال سازی",
     "غیر فعال"] or owner_lock != "osine1":
