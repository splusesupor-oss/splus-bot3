...
# اخراج کاربر با ریپلای
            if clean_text == "اخراج":
                try:
                    ...
                    print("CHAT TYPE:", type(chat_id))
                    print("TARGET:", target_user)
                    print("TARGET ID:", getattr(target_user,"id",None))

                    try:
                        entity = await event.get_chat()
                        print("ENTITY:", entity)
                        print("ENTITY TYPE:", type(entity))
                        print("ENTITY TITLE:", getattr(entity,"title",None))
                    except Exception as ee:
                        print("ENTITY ERROR:", ee)

                    try:
                        me = await self.client.get_me()
                        print("ME:", me)
                    except Exception as meerr:
                        print("ME ERROR:", meerr)
...
# KICK SPlus auto detect group type
from splusthon import helpers, types
from splusthon.tl import functions

entity = await self.client.get_input_entity(chat_id)
user_entity = await self.client.get_input_entity(target_user)

etype = helpers._entity_type(entity)

if etype == helpers._EntityType.CHAT:
    await self.client(
...
elif etype == helpers._EntityType.CHANNEL:
    # kick from megagroup/channel
    await self.client(
