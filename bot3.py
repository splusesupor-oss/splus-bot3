#!/usr/bin/env python3
"""Instance سوم ربات — فقط یک launcher است و هیچ کد رباتی duplicate نمی‌کند.

کار این فایل فقط این است که **قبل از import شدن هر ماژول پروجه** متغیر
``BOT_INSTANCE`` را روی ``bot3`` قرار دهد و بعد همان entrypoint واقعی
...
``modules/runtime_paths.py`` بر اساس همین instance جدا می‌شوند:
    ...
    python3 bot3.py  ->  ~/.local/share/soroush-bot-bot3/
...
قفل PID (bot.pid) به‌صورت per-instance است؛ main هرگز bot3 را مسدود
نمی‌کند و بالعکس.
...
os.environ["BOT_INSTANCE"] = "bot3"

import asyncio

# import AFTER تعیین instance: بدنه‌ی main.py (تعیین dotenv و importهای
# پروجه) حالا با BOT_INSTANCE=bot3 اجرا می‌شود. بلاک if __name__ ==
