        ...
        try:
            ...
            if clean_text == "سکوت":
                try:
                    ...
                    # گرفتن کاربر ریپلای شده مخصوص SPlus
                    reply_id = getattr(event.message.reply_to, "reply_to_msg_id", None)
                    ...
                    try:
                        ...
                        if not original or not original.from_id:
                            await event.reply("❌ کاربر ریپلای شده پیدا نشد")
                            return
                        ...
                        self.logger.log_error(f"خطا در گرفتن پیام ریپلای: {e}")
                        await event.reply("❌ پیدا کردن کاربر ناموفق بود")
                        return

                    # تشخیص ادمین‌های گروه
                    ...
                    await event.reply("❌ خطا در سکوت کاربر")
                        ...
                        if username and group_link:
                            ...
                            async for d in self.client.iter_dialogs():
                                if hasattr(d.entity, "title"):
                                    # انتخاب گروهی که ربات داخل آن است
                                    if group_link and d.message:
                                        group = d.entity
                                        break
