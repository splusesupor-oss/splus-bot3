...
user_tracker = UserTracker(threshold=4)
...
"یکی گفت آینده‌ات روشنه، گفتم فقط قبض برقش زیاده 😂"
        ...
        try:
            if check_fill(chat_id, user_id, clean_text):
                await event.reply("✅ جواب درست بود!\n⭐ امتیاز گرفتی")
                return
        except Exception as e:
            bot.logger.log_error(f"خطای جای خالی: {e}")
        ...
        # صفر کردن تخلفات کاربر از پیوی
        if event.is_private and message_text.strip().startswith("صفر"):
                try:
                    import re
                    import json
                    ...
                    all_counts = bot.tracker.get_all_counts()

                    for gid, users in all_counts.items():
                        if str(user_id) in users:
                            bot.tracker.reset_count(int(gid), user_id)
                            try:
                    ...
                    if not reset_groups:
                        await event.reply("❌ این کاربر تخلف ثبت شده‌ای ندارد")
                        return

                    await event.reply("✅ تخلفات کاربر صفر شد")

                except Exception as e:
                    await event.reply(f"❌ خطا در صفر کردن تخلفات:\n{e}")
...
"لیست بازی\n\n"
                ...
                "👑 دستورات ادمین‌ها:\n\n"
"⚠️ اخطار کاربر:\n"
"برای ارسال اخطار به کاربر، روی پیام او ریپلای کنید و بنویسید:\n"
"اخطار\n\n"
"🔤 فیلتر کلمات گروه:\n"
        ...
        if clean_text.startswith("پاک "):
            try:
                ...
                if not is_admin(chat_id, sender_username):
                    print("DEBUG WARNING ADMIN:", chat_id, sender_username, is_admin(chat_id, sender_username))
...
# اخطار کاربر با ریپلای
        ...
        if clean_text == "سکوت":
            try:
                ...
                await event.reply(f"❌ خطا در سکوت:\n{e}")

            return

        user_msgs = []
        for m in bot.flood_messages.get(chat_id, []):
            if m[0] == user_id:
                user_msgs.append(m)
